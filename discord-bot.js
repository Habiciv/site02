...
async function send(path, payload) {
  const response = await fetch(`${site}${path}`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-rng-sync-key': process.env.SYNC_API_KEY || '' }, body: JSON.stringify(payload) });
  if (!response.ok) throw new Error(`Sincronização falhou: ${response.status}`);
}
async function syncMember(member) {
  await send('/api/sync/member', { discordId: member.id, username: member.user.tag, roles: [...member.roles.cache.values()].map(r => r.name), joinedAt: member.joinedAt?.toISOString() });
}
